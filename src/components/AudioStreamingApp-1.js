...
const AudioStreamingApp = () => {
  ...
  const resetAudio = async (startRecording) => {
    ...
    console.log('xyz', audioRecorderRef.current);
    ...
    if (startRecording) {
      ...
      console.log("Audio buffer:", bufferRef.current);

    }
  };

  const handleRealtimeMessages = async () => {
    console.log('realtimeStreamingRef.current' , realtimeStreamingRef.current);
    for await (const message of realtimeStreamingRef.current.messages()) {
      let consoleLog = "" + message.type;
      console.log(consoleLog);

      console.log('message.delta', message.delta);
